import { EventName, IModelOptions } from "../../interfaces/interfaces";
import Observer from "../observer";

describe("observer", () => {
  jest.mock("../observer");
  const subscriber1Fn = jest.fn((e: MouseEvent) => "start");
  let observer: Observer;
  const subscriber = {
    eventName: EventName.clickedHandle,
    function: subscriber1Fn,
  };
  const subscriber2Fn = jest.fn((model: IModelOptions) => "start");
  const subscriber2 = {
