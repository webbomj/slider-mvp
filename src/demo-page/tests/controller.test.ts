...
describe('Controller', () => {
  ...
  afterEach(() => {
    const controllerNode: HTMLElement | null = document.querySelector('controller');
    if (controllerNode) {
  ...
  test('Should return 11 inputs (6 number input, 5 checkbox input)', () => {
    ...
    const inputsNumber = document.querySelectorAll("#controller input[type='number']");
    const inputsCheckbox = document.querySelectorAll("#controller input[type='checkbox']");
  ...
  test('Should change value for input', () => {
    controller.handleInputClick = jest.fn(() => {});
    ...
    expect(controller.handleInputClick).toBeCalled();
  });

  test('Should inputsHandler called', () => {
    const inputsHandler = jest.spyOn(controller, 'handleInputClick');
    const fullUpdate = jest.spyOn(slider, 'fullUpdate');

    let event2 = new Event('click');
    controller.handleInputClick(event2);

    expect(inputsHandler).toBeCalled();

    $('[name="isProgressBar"]').trigger($.Event('click'));
    const progressBar: HTMLInputElement | null = document.querySelector('[name="isProgressBar"]');
    let value;
    if (progressBar) {
